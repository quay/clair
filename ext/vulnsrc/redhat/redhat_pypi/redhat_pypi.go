...
func init() {
	vulnsrc.RegisterUpdater("redhat_pypi", &updater{})
}
...
func (u *updater) Update(datastore database.Datastore) (resp vulnsrc.UpdateResponse, err error) {
	...
	for rd := rangeDate(start_date, end_date); ; {
		...
		for page_flag && page < 100 {
			...
			redhat_api_url := redhat_api + time_format[:4] + time_format[5:7] + time_format[8:] + "?page=" + strconv.Itoa(page) + "&ecosystem=pypi"
