...
func NewHttpTransport(ctx context.Context, conf config.Config, indexer indexer.Service, matcher matcher.Service) (*HttpTransport, error) {
...
func (t *HttpTransport) configureDevMode() error {
	...
	t.Handle(VulnerabilityReportPath, matcher.VulnerabilityReportHandler(t.matcher, t.indexer))
	t.Handle(IndexAPIPath, indexer.IndexHandler(t.indexer))
	t.Handle(IndexReportAPIPath, indexer.IndexReportHandler(t.indexer))
	t.Handle(StateAPIPath, indexer.StateHandler(t.indexer))
	return nil
}
