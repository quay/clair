...
var (
	errSkipMutation     = errors.New("must not read or write any tokens when skipping")
...
type (
	typedMarshalers           = typedArshalers[jsontext.Encoder]
	typedUnmarshalers         = typedArshalers[jsontext.Decoder]
	typedArshalers[Coder any] struct {
		nonComparable
...
type (
	typedMarshaler           = typedArshaler[jsontext.Encoder]
	typedUnmarshaler         = typedArshaler[jsontext.Decoder]
	typedArshaler[Coder any] struct {
		typ     reflect.Type
