...
import (
	...
	"github.com/quay/clair/v4/internal/codec/internal/json"
	"github.com/quay/clair/v4/internal/codec/internal/json/jsontext"
)
...
func v1ManifestMarshal(enc *jsontext.Encoder, v *claircore.Manifest) error {
	...
	enc.WriteToken(hashKey)
	enc.WriteToken(jsontext.String(v.Hash.String()))

	if v.Layers != nil {
		enc.WriteToken(layersKey)
...
func v1LayerMarshal(enc *jsontext.Encoder, v *claircore.Layer) error {
	...
	enc.WriteToken(uriKey)
	enc.WriteToken(jsontext.String(v.URI))

	enc.WriteToken(headersKey)
...
func v1IndexReportMarshal(enc *jsontext.Encoder, v *claircore.IndexReport) error {
	...
	enc.WriteToken(reporthashKey)
	enc.WriteToken(jsontext.String(v.Hash.String()))

	enc.WriteToken(stateKey)
	enc.WriteToken(jsontext.String(v.State))

	enc.WriteToken(successKey)
	enc.WriteToken(jsontext.Bool(v.Success))

	if e := v.Err; e != "" {
		enc.WriteToken(errKey)
		enc.WriteToken(jsontext.String(e))
	}
...
func v1PackageMarshal(enc *jsontext.Encoder, v *claircore.Package) error {
	...
	for _, f := range fs {
		...
		enc.WriteToken(f.Key)
		enc.WriteToken(jsontext.String(f.Value))
	}

	if v.NormalizedVersion.Kind != "" {
		enc.WriteToken(normVersionKey)
	...
	if v.CPE.Valid() == nil {
		enc.WriteToken(cpeKey)
		enc.WriteToken(jsontext.String(v.CPE.String()))
	}

	if src := v.Source; src != nil {
		enc.WriteToken(sourceKey)
...
func v1EnvironmentMarshal(enc *jsontext.Encoder, v *claircore.Environment) error {
	...
	if v.IntroducedIn.Algorithm() != "" {
		enc.WriteToken(introducedKey)
		enc.WriteToken(jsontext.String(v.IntroducedIn.String()))
	}

	if len(v.RepositoryIDs) != 0 {
		enc.WriteToken(repositoryIDsKey)
		enc.WriteToken(jsontext.BeginArray)
		defer enc.WriteToken(jsontext.EndArray)
...
func v1VulnerabilityMarshal(enc *jsontext.Encoder, v *claircore.Vulnerability) error {
	...
	enc.WriteToken(normSeverityKey)
	enc.WriteToken(jsontext.String(v.NormalizedSeverity.String()))

	if !v.Issued.IsZero() {
		enc.WriteToken(issuedKey)
	...
	if v.ArchOperation != claircore.ArchOp(0) {
		enc.WriteToken(archOpKey)
		enc.WriteToken(jsontext.String(v.ArchOperation.String()))
	}
