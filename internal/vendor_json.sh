...
cat <<'.' >"$sedscript"
s/go:build goexperiment.jsonv2$/go:build !goexperiment.jsonv2 || !go1.27/;
...
go tool alias_gen "encoding/json/v2"       "$JSONROOT"
go tool alias_gen "encoding/json/jsontext" "$JSONROOT/jsontext"

echo generating gitattributes
echo '** linguist-generated' > "$JSONROOT/.gitattributes"
