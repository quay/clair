...
func New(ctx context.Context, conf *config.Config, indexer indexer.Service, matcher matcher.Service, notifier notifier.Service) (http.Handler, error) {
	...
	// Note that the arms have a conditional at the end to make them work like a
	// C switch.
	switch conf.Mode {
	case config.IndexerMode, config.ComboMode:
		if indexer == nil {
	...
	case config.MatcherMode:
		...
		v1 := NewMatcherV1(ctx, prefix, matcher, indexer, conf.Matcher.CacheAge, traceOpt)
