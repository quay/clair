...
type OSInfo interface {
	...
	// should return glibc and 3.2
	// This is based on the assumption that the distributions generate the comments
	// automatically and they won't change (I know, not very reliable...)
	ParsePackageNameVersion(comment string) (string, string)
	// ParseOsVersion should, given a comment in a criterion, return the version
	// of the Operating System.
	// For example, if the comment is
	//   SUSE Linux Enterpise Server 12 is installed
	// should return 12
	// This is based on the assumption that the distributions generate the comments
	// automatically and they won't change it (I know, not very reliable...)
	ParseOsVersion(comment string) string
	// Given a line, parse for the xml file that contains the oval definition and
	// returns the filename
	// For example if the line contains
	//	 com.redhat.rhsa-2003.xml, this will be return
	// This is being used in conjunction with OvalUri (see below). Oval Uri contains
	// a list of files, and you need ParseFilenameDist to get the right ones
	ParseFilenameDist(line string) string
	// OvalUri returns the url where the oval definitions are stored for a given
	// distributions. See examples:
	//   https://www.redhat.com/security/data/oval/
	//   http://ftp.suse.com/pub/projects/security/oval/
	OvalURI() string
	// DistName returns the distribution name. Mostly used for debugging purposes
	DistName() string
	...
	// SecToken returns a string that is compared with the value of reference.source
	// in order to know if that is a security reference for, for example, using
	// its url value
	// Example return values: CVE, RHSA
	SecToken() string
	//Namespace stores the namespace that will be used in clair to store the vulnerabilities
	Namespace() string
}
...
func (f *OvalFetcher) FetchUpdate(datastore database.Datastore) (resp updater.FetcherResponse, err error) {
	...
	if len(distList) > 0 {
		resp.FlagName = f.OsInfo.DistName() + "updater"
		resp.FlagValue = distList[len(distList)-1]
	} else {
		log.Debug("no #{f.OsInfo.DistroName()}")
	}
